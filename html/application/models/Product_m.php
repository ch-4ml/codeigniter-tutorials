<?
    class Product_m extends CI_Model     // 모델 클래스 선언
    {
        ...
        public function getrow($no) {
            $sql = "select product12.*, category12.name12 as category_name from product12 left join category12 on product12.category_no12 = category12.no12 where product12.no12 = $no";
        ...
        {
            $where = array("no12" => $no);
            return $this->db->update("product12", $row, $where);
        }

        public function deleterow($no) {
            $sql = "delete from product12 where no12 = $no";
